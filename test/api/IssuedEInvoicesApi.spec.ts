...
import { IssuedEInvoicesApi } from '../../src/api/issued-einvoices-api'
import { SendEInvoiceRequest, SendEInvoiceResponse, VerifyEInvoiceXmlResponse } from '../../src/models'

const sandbox = require('sinon').createSandbox()

const instance = new IssuedEInvoicesApi()

const sendEInvoiceResponse: SendEInvoiceResponse = { data: { name: 'CARICATO', date: '2021-08-23 10:38:03' } }
const sendEInvoiceStub = sandbox.stub(instance, 'sendEInvoice').returns(sendEInvoiceResponse)

const verifyEInvoiceXmlResponse: VerifyEInvoiceXmlResponse = { data: { success: true } }
sandbox.stub(instance, 'verifyEInvoiceXml').returns(verifyEInvoiceXmlResponse)
...
describe('IssuedEInvoicesApi', function () {
  describe('sendEInvoice', function () {
    it('should call sendEInvoice successfully', function (done) {
      const opts: SendEInvoiceRequest = { data: { withholding_tax_causal: 'causale' } }
      const response = instance.sendEInvoice(2, 12345, opts)
      const expectedJson = JSON.stringify(sendEInvoiceResponse)
      const actualJson = JSON.stringify(response)
      expect(actualJson).to.equal(expectedJson)
      expect(sendEInvoiceStub.getCall(0).args[2]).to.equal(opts)
      done()
    })
  })
  describe('verifyEInvoiceXml', function () {
    it('should call verifyEInvoiceXml successfully', function (done) {
      const response = instance.verifyEInvoiceXml(2, 12345)
      const expectedJson = JSON.stringify(verifyEInvoiceXmlResponse)
      const actualJson = JSON.stringify(response)
      expect(actualJson).to.equal(expectedJson)
      done()
    })
  ...
  describe('getEInvoiceXml', function () {
    it('should call getEInvoiceXml successfully', function (done) {
      const response = instance.getEInvoiceXml(2, 12345)
