...
import { SendEInvoiceRequest, SendEInvoiceResponse, VerifyEInvoiceXmlResponse } from '../../src/models';
  ...
  var instance = new IssuedEInvoicesApi();

  var sendEInvoiceResponse: SendEInvoiceResponse = {"data":{"name":"CARICATO","date":"2021-08-23 10:38:03"}};
  var sendEInvoiceStub = sandbox.stub(instance, "sendEInvoice").returns(sendEInvoiceResponse);

  var verifyEInvoiceXmlResponse: VerifyEInvoiceXmlResponse = {"data":{"success":true}};
  ...
  describe('IssuedEInvoicesApi', function() {
    describe('sendEInvoice', function() {
      it('should call sendEInvoice successfully', function(done) {
        var opts: SendEInvoiceRequest = {"data":{"withholding_tax_causal":"causale"}}
        var response = instance.sendEInvoice(2, 12345, opts);
        var expectedJson = JSON.stringify(sendEInvoiceResponse);
    ...
    describe('verifyEInvoiceXml', function() {
      it('should call verifyEInvoiceXml successfully', function(done) {
        var response = instance.verifyEInvoiceXml(2, 12345);
        var expectedJson = JSON.stringify(verifyEInvoiceXmlResponse);
        var actualJson = JSON.stringify(response);
        expect(actualJson).to.equal(expectedJson);
        done();
      });
    ...
    describe('getEInvoiceXml', function() {
      it('should call getEInvoiceXml successfully', function(done) {
        var response = instance.getEInvoiceXml(2, 12345);
